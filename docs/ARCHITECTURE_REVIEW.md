...
Compose의 controller와 worker는 내부 네트워크에만 연결합니다. 로컬 모델 접근은 별도 relay를 거치며, relay는 고정된 로컬 주소의 모델 목록과 추론 경로만 허용합니다. 임의 URL·모델 삭제·파일 업로드·검색 프록시는 제공하지 않습니다. relay에는 증거·사건 볼륨이 없고 입력·응답 크기를 제한합니다. 네트워크로부터 완전히 분리된 실행과 모델 접근용 relay의 제한된 외부 연결은 구분합니다.
