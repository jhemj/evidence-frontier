...
function renderTimeline(data, investigation) {
  ...
  root.innerHTML = `<div class="tl-heading"><div><span class="tl-eyebrow">EVIDENCE IN TIME</span><h2>흔적이 사건으로 연결되는 순간</h2><p>기록의 흐름을 따라, 근거와 판단의 변화를 한눈에.</p></div><span class="tl-live ${running?"is-live":""}"><i></i>${running?"조사 중 · 자동 갱신":"현재 조사 결과"}</span></div>
    ...
    ${reviewSummary}
    ${data.coverage_map?`<details class="tl-coverage"><summary>자료 처리 범위 · 중요 미처리 ${data.coverage_map.critical_gap_count}개</summary><p>읽은 자료와 AI가 검토한 단서는 서로 다른 범위입니다.</p>${Object.entries(data.coverage_map.families).map(([name,f])=>`<p><strong>${esc(name)}</strong> 발견 ${f.discovered} · 전체 읽음 ${f.fully_scanned} · 텍스트 파싱 ${f.text_parsed} · 미처리 ${f.unprocessed}</p>`).join('')}<p>${esc(data.coverage_map.time_coverage)}</p>${(data.coverage_map.source_matrix||[]).map(r=>`<p><strong>${esc(({authentication:'인증',accounts_privilege:'계정·권한',persistence:'지속성',execution:'실행',network:'통신',applications:'애플리케이션',unclassified:'미분류',memory:'메모리',deleted_unallocated:'삭제·미할당',journal_packet_semantics:'저널·패킷 의미 분석',windows_evidence_parsers:'Windows 증거 파서'})[r.area]||r.area)}</strong> · ${esc(({unsupported:'미지원',enumeration_incomplete:'열거 불완전',not_observed_in_enumerated_scope:'열거 범위에서 미발견',scope_recorded:'범위 기록됨'})[r.status]||r.status)}${r.discovered!==undefined?` · 발견 ${r.discovered} · 미선정 ${r.not_selected||0} · 부분 ${r.partial||0} · 미지원 형식 ${r.format_unparsed||0}`:''}</p>`).join('')}</details>`:''}
