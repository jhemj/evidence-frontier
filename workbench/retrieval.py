...
def search(run, image, manifest, request):
    ...
    def continuation():
        return base64.urlsafe_b64encode(json.dumps({'binding':binding,'file':fi,'offset':offset,'line':line_number}).encode()).decode()
    next_cursor=''
    while fi < len(files):
        ...
        with path.open('rb') as stream:
            ...
            while True:
                ...
                if query in text.casefold():
                    if source:
                        hit=raw.lower().find(query.encode('utf-8').lower())
                        window_start=max(0,hit-1500)
                        window=raw[window_start:window_start+6000]
                        item={'type':'linux_literal_match','timestamp':None,
                            'source_location':f"{image.name}:byte:{source['partition_offset']}:{source['path']}:inode:{source['inode']}:offset:{source.get('source_offset',0)+before}",
